...
export function TrendChart({ data }: { data: TimePoint[] }) {
  return (
    <div className="h-64 rounded-xl border border-slate-800/60 bg-slate-900/60 p-4">
      <div className="mb-2 text-sm text-muted-foreground">Engagement vs Time</div>
      <ResponsiveContainer width="100%" height="100%">
        <AreaChart data={data} margin={{ left: -20, right: 10, top: 10, bottom: 0 }}>
          ...
          <YAxis stroke="#64748b" width={40} tickLine={false} axisLine={{ stroke: "#334155" }} />
          <RTooltip contentStyle={{ background: "#0b1220", border: "1px solid #1e293b" }} />
          <Area type="monotone" dataKey="engagement" stroke="#22d3ee" strokeWidth={2} fill="url(#colorEng)" />
        </AreaChart>
      </ResponsiveContainer>
    </div>
  );
...
export function StateBarChart({ data }: { data: StateBar[] }) {
  return (
    <div className="h-64 rounded-xl border border-slate-800/60 bg-slate-900/60 p-4">
      <div className="mb-2 text-sm text-muted-foreground">Accounts Flagged per State</div>
      <ResponsiveContainer width="100%" height="100%">
        <BarChart data={data} margin={{ left: 0, right: 10, top: 10, bottom: 0 }}>
          <XAxis dataKey="state" stroke="#64748b" tickLine={false} axisLine={{ stroke: "#334155" }} />
          <YAxis stroke="#64748b" width={40} tickLine={false} axisLine={{ stroke: "#334155" }} />
          <RTooltip contentStyle={{ background: "#0b1220", border: "1px solid #1e293b" }} />
...
export function SentimentDonut({ data }: { data: DonutSlice[] }) {
  return (
    <div className="h-64 rounded-xl border border-slate-800/60 bg-slate-900/60 p-4">
      <div className="mb-2 text-sm text-muted-foreground">Sentiment Breakdown</div>
      <ResponsiveContainer width="100%" height="100%">
        <PieChart>
