...
function genAlerts(seed = Date.now()): AlertItem[] {
  ...
  return Array.from({ length: 30 }).map((_, i) => ({
    id: `al${i}`,
    priority: p[Math.floor(rand() * 3)],
    title: `Coordinated spread detected on ${genAntiTag(rand)}`,
    evidence: genEvidenceUrl(rand),
  }));
}
        ...
        <DialogContent className="bg-slate-900 border-slate-800 max-w-2xl">
          ...
          {open && (
            <div className="space-y-2 text-sm">
              <div>
                Priority: <strong>{open.priority}</strong>
              </div>
              <div>
                Evidence:{" "}
                <a
              ...
              <div className="text-muted-foreground">
                Linked posts, accounts, propagation timeline would appear here.
              </div>
            </div>
          )}
        </DialogContent>
