...
export default function TopK() {
  ...
  const { accounts, hashtags, posts } = useMemo(
    () => generateTopK(seed),
    [seed],
  );
  ...
  const pst = posts.filter((p) =>
    p.preview.toLowerCase().includes(q.toLowerCase()),
  );
    ...
    <AppLayout>
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="flex gap-2">
          <Button
            size="sm"
            onClick={() => exportCSV("accounts.csv", acc as any)}
          >
            Export Accounts
          </Button>
          <Button
            size="sm"
            onClick={() => exportCSV("hashtags.csv", hash as any)}
          >
            Export Hashtags
          </Button>
          <Button size="sm" onClick={() => exportCSV("posts.csv", pst as any)}>
            Export Posts
          </Button>
        </div>
      </div>
              ...
              <tbody>
                {hash.slice(0, 20).map((h) => (
                  <tr key={h.id} className="border-t border-slate-800/70">
                    <td className="pr-2">
                      <div className="truncate" title={h.tag}>
                        {h.tag}
                      </div>
                    </td>
                    <td className="text-right tabular-nums whitespace-nowrap w-24">
                      {(h.count as number).toLocaleString()}
                    </td>
                    <td className="text-right whitespace-nowrap w-20">
                      {h.radius}
                    </td>
                  </tr>
                ))}
              ...
              <tbody>
                {pst.slice(0, 20).map((p) => (
                  <tr key={p.id} className="border-t border-slate-800/70">
                    <td>
                      <div
                        className="truncate max-w-[220px] md:max-w-[240px] xl:max-w-[260px]"
                        title={p.preview}
                      >
                        {p.preview}
                      </div>
